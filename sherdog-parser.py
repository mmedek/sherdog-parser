...
class Fighter(object):
    ...
    def __init__(self):
        ...
        self.pro_range = (
            None  # selector: selecting range to pro fights exclusively, None by default
        )
    ...
    def grab_result_data(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing result data for {self.name}, the data might be missing!"
            )
        else:
    ...
    def grab_opponents(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing opponent data for {self.name}, the data might be missing!"
            )
        else:
    ...
    def grab_events(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing event data for {self.name}, the data might be missing!"
            )
        else:
            self.events = events
            return events
    ...
    def grab_events_date(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing event dates for {self.name}, the data might be missing!"
            )
        else:
    ...
    def grab_judges(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing judges data for {self.name}, the data might be missing!"
            )
        else:
            self.judges = judges
            return judges
    ...
    def grab_method(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing methods data for {self.name}, the data might be missing!"
            )
        else:
            self.method = methods
            return methods
    ...
    def grab_rounds(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing rounds data for {self.name}, the data might be missing!"
            )
        else:
            self.rounds = rounds
            return rounds
    ...
    def grab_time(self):
        ...
        try:
            ...
            logging.info(
                f"Attribute Error while grabbing time data for {self.name}, the data might be missing!"
            )
        else:
            self.time = time
            return time
        ...
        with open(f"{filename}.csv", "a", newline="", encoding="ISO-8859-1") as csvfile:
            ...
            for index in range(len(self.result_data)):
                ...
                try:
                    ...
                    print(
                        f"Coding error while attempting to save date for {self.name}, line was dropped!"
                    )
    ...
    def save_to_json(self, filename):
        ...
        fighter_dictionary = {
            self.name: []
        }  # initializing dictionary that will be passed into json file.
    ...
    def scrape_fighter(self, filetype, filename, fighter_index=None, fighter_page=None):
        ...
        if fighter_index is not None:
            ...
            print(
                "Error, please pass fighter's index, or fighter's page in order to proceed."
            )
        self._set_resource()
        self._set_soup()
        if (
            self.set_name() != AttributeError
        ):  # checking if there is existing name for a fighter instance.
            self.set_pro_fights()
            ...
            self.grab_time()
            if (
                self.get_validation() != TypeError
            ):  # if there was an empty list while validating data,
...
def scrape_all_fighters(filename, filetype="csv"):
    ...
    while (
        ...
        scrapped_data = F.scrape_fighter(
            filetype, filename, fighter_index=fighter_index
        )
        if scrapped_data is True:
            fail_counter = 0  # resetting fail counter after finding valid page(index) for a fighter.
        else:
            fail_counter += (
                1  # incrementing fail counter if there was no data for certain index.
            )
    ...
    for gender_index in range(1, 3):
        ...
        while True:
            resource = requests.get(
                f"https://www.ufc.com/athletes/all?filters%5B0%5D=status%3A23&"
            ...
            if (
                len(fighter) == 0
            ):  # if page is empty = there are no fighters left, current gender index is done.
                break
            else:
                for index in range(len(fighter)):
                    name = (
                        fighter[index]
                        .find("span", class_="c-listing-athlete__name")
                        .get_text()
                        .strip()
                    )
                    ...
                    try:
                        nickname = (
                            fighter[index]
                            .find("span", class_="c-listing-athlete__nickname")
                            .div.get_text()
                        )
    ...
    if save == "yes":
        if filetype == "csv":
            ...
            with open("ufc-roster.csv", "w", newline="") as csvfile:
                ...
                init_writer.writerow(headers)
            with open(
                "ufc-roster.csv", "a", newline="", encoding="ISO-8859-1"
            ) as csvfile:
                writer = csv.writer(csvfile, delimiter=",")
                for index in range(len(ufc_roster["men"])):
                    try:
                        writer.writerow(
                            [
                                ufc_roster["men"][index][0],
                                ufc_roster["men"][index][1],
                                ufc_roster["men"][index][2],
                            ]
                        )
                    except UnicodeEncodeError:
                        print(
                            f'Unfortunately record containing {ufc_roster["men"][index]} '
                ...
                for index in range(len(ufc_roster["women"])):
                    try:
                        writer.writerow(
                            [
                                ufc_roster["women"][index][0],
                                ufc_roster["women"][index][1],
                                ufc_roster["women"][index][2],
                            ]
                        )
                    except UnicodeEncodeError:
                        print(
                            f'Unfortunately record containing {ufc_roster["women"][index]} '
                            f"dropped due to UnicodeEncodeError!"
                        )
        elif filetype == "json":
...
def scrape_list_of_fighters(fighters_list, filename, filetype="csv"):
    ...
    def search_fighter(fighter_tuple):
        ...
        """
        res_1 = requests.get(
            f"https://www.sherdog.com/stats/fightfinder?SearchTxt={fighter_tuple[0]}"
        )
        res_2 = requests.get(
            f"https://www.sherdog.com/stats/fightfinder?SearchTxt={fighter_tuple[0]}"
    ...
    def soup_selector(request):
        ...
        css_selector = soup.select(
            "body > div.container > div:nth-child(3) > div.col_left > section:nth-child(2) > "
    ...
    for fighter in fighters_list:
        search_results = search_fighter(
            fighter
        )  # variable that stores different searching results.
        find_fighter = search_results[0]  # assigning first result to variable.
        selector = soup_selector(
            find_fighter
        ...
        if results == IndexError:
            logging.info(
                f"Error occurred with {fighter}, please check carefully "
                f"if there is no mistake in fighter name!"
            )
        else:
            if len(results) == 1:
                create_fighter_instance(
                    results
                ...
                if results == IndexError:
                    try:
                        find_fighter = search_results[
                            2
                        ]  # assigning third result to variable.
                    except KeyError:
                        print(
                            "Search engine tried to narrow down findings by using weight-class filter, apparently "
                        ...
                        if results == IndexError:
                            logging.info(
                                f"Error occured with {fighter}, please check carefully if there is no mistake "
                                f"in nickname!"
                            )
                        else:
                            if len(results) == 1:
                    ...
                    if results == IndexError:
                        ...
                        if len(results) == 1:
                            ...
                            try:
                                ...
                                print(
                                    f"Search engine tried to narrow down findings by name & nickname, apparently this"
                                ...
                                if results == IndexError:
                                    ...
                                    if len(results) == 1:
                                        ...
                                        try:
                                            find_fighter = search_results[
                                                3
                                            ]  # assigning fourth result to variable.
                                        except KeyError:
                                            print(
                                                f"Search engine tried to narrow down findings by using all filters, "
